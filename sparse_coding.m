function [B S stat] = sparse_coding(Y_total, num_bases, beta, sparsity_func, epsilon, num_iters, batch_size, fname_save, pars, Binit, resample_size)
% Fast sparse coding algorithms
%
%    minimize_B,S   0.5*||Y - B*S||^2 + beta*sum(abs(S(:)))
...
if exist('resample_size', 'var') && resample_size
    assert(size(Y_total,2) > resample_size);
    Y = Y_total(:, randsample(size(Y_total,2), resample_size));
else
    Y = Y_total;
end

pars.patch_size = size(Y,1);
pars.num_patches = size(Y,2);
...
while t < pars.num_trials
    ...
    if exist('resample_size', 'var') && resample_size
        fprintf('resample X (%d out of %d)...\n', resample_size, size(Y_total,2));
        Y = Y_total(:, randsample(size(Y_total,2), resample_size));
    end
    
    % Take a random permutation of the samples
    indperm = randperm(size(Y,2));
    
    for batch=1:(size(Y,2)/pars.batch_size),
        ...
        if t==1 || ~pars.reuse_coeff
            ...
            if strcmp(pars.sparsity_func, 'L1') || strcmp(pars.sparsity_func, 'LARS') || strcmp(pars.sparsity_func, 'FS')
                tic
                % use the feature sign to learn the cofficients
